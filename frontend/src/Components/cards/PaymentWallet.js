...
import { createPaymentWalletIntent } from "../../functions/stripe";
...
const PaymentWallet = () => {
    ...
    const [price, setPrice] = useState(localStorage.getItem("walletpayment"));
    ...
    useEffect(() => {
        createPaymentWalletIntent(price).then((res) => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (payload.error) {
            ...
            setProcessing(false);
            setSucceeded(true);
            window.localStorage.removeItem("walletpayment");
